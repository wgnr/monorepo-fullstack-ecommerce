        ...
        if (["/order", "/cart"].includes(msgResponse.msg.toLowerCase())) {
          try {
            if (msgResponse.msg.toLowerCase() === "/order")
              msgResponse.msg = JSON.stringify(
                await OrdersService.getOrdersByUserId(toUser),
                null,
                2
              );
            else if (msgResponse.msg.toLowerCase() === "/cart")
              msgResponse.msg = JSON.stringify(
                await CartsService.getFullPopulatedById(user.currentCart),
                null,
                2
              );
          } catch (err) {
