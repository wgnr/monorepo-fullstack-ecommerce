...
import { IProduct, IProductDocument, IProductNew } from "@models/entities/products/products.interfaces"
...
class ProductService {
  ...
  async getPopulatedByProductId(productId: string) {
    ...
    const optionValuesIds = [...new Set(
      product.variants
        .map(
          variant => variant.options.map((option: ObjectId) => String(option))
        )
        .flat()
    )]

    const optionsResponses = await Promise.all(optionValuesIds.map(
      valueId => OptionsService.getOptionNameAndValueByValueId(valueId)
    ))

    const responseProduct = product.toJSON()
    responseProduct.variants.forEach(variant => {
      // @ts-ignore
      variant.options = variant.options.map((option: ObjectId) => optionsResponses.find(
  ...
  async createProduct(product: IProductNew) {
    ...
    if (variants.length > 0) {
      ...
      await Promise.all(variants.map(({ options }) => {
    ...
    if (variants.length > 0) {
      const newVariants = await Promise.all(variants.map(
        ({ options, stock }) =>
  ...
  async addVariant(productId: string, variant: IVariantBase) {
    ...
    const newVariant =
      await VariantsService.create(productId, true, stock, options)
