...
class OptionsService {
  ...
  async validateIfOptionsValuesAreUsed(valuesId: string[]) {
    ...
    if (valuesIdInVariants.length > 0) {
      throw new ValidationException(`Options values are still being used in products. Can't delete until they are removed. Ids: ${valuesIdInVariants.join(", ")}`)
    }
  }
