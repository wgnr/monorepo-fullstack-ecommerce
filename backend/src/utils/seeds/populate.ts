...
(async () => {
  try {
    ...
    const orders = await Promise.all(mockOrders(carts).map(payload =>
      post(token, SEED_SERVER_API_URL, PATHS.ORDERS, payload)
    ))
    const payedOrder = await post(token, SEED_SERVER_API_URL, PATHS.ORDERS + "/" + orders[0]._id, mockPay)
