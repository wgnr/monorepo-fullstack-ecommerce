...
UsersSchema.methods.isValidPassword =
  async function (this: IUserDocument, password: string) {
    return await compare(password, this.password!).catch(e => false)
  }
// TODO checkear si esto es compatible con el get undefined
// quiza tenga que acceder con el usersSchema.path("password") (es una instancia) this.path("")?

export const UsersModel: Model<IUser> = model<IUser>(
  collectionName,
  UsersSchema
);